...
void tick() {
    ...
    if (clock_count % 1000 == 0) {
        printf("Clock %d: clk=%d, reset=%d\n", clock_count, display->clk, display->reset);
    }
    // 下降沿
    wait_10ns();
    main_time++;
...
//    debug_count++;
    ...
    static int debug_count = 0;
    if (debug_count % 1000 == 0) {
        printf("Counters - X: %d, Y: %d, Pre_HSYNC: %d, Pre_VSYNC: %d\n", 
               coord_x, coord_y, pre_h_sync, pre_v_sync);
    }
    debug_count++;    
    ...
    if (debug_count % 5000 == 0) {
        printf("VGA Signals - HSYNC: %d, VSYNC: %d, RGB: 0x%x, X: %d, Y: %d\n", 
               display->h_sync, display->v_sync, display->rgb, coord_x, coord_y);
    }
    debug_count++;
